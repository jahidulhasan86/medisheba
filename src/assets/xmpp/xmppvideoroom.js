var XMPPVideoRoom = (function() {
	...
	*/
	var XMPPVideoRoom = function XMPPVideoRoom (xmppUrl, srvurl, bus, roomName, token) {	
		this.xmppUrl     = xmppUrl;
		this.srvurl      = srvurl || "//"+window.location.hostname+":"+window.location.port;
		this.sessionList = {};
		this.bus = bus;
		this.connection = new Strophe.Connection("https://" + this.xmppUrl + "/http-bind?room="+roomName+"&token="+token);
	...
	*/
	XMPPVideoRoom.prototype.join = function(roomid, url, name, password) {
		...
		this.getConnection(roomid).then( (connection) => {
				if (connection.disco) {
					connection.disco.addIdentity('client', 'https://videoengine.sensor.buzz');
